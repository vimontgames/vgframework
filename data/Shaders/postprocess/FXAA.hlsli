...
float4 FXAA(Texture2D src, float2 uv, DisplayMode _displayMode)
{
    ...
    float4 result = FxaaPixelShader( uv,                             // pos                              {xy} = center of pixel
    ...
    #if _TOOLMODE
    if (DisplayMode::PostProcess_FXAAEdges == _displayMode)
    {
        float4 color = src.SampleLevel(linearClamp, uv, 0);
        if (any(color.rgb != result.rgb))
