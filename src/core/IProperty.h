...
namespace vg::core
{
    ...
    vg_enum_class(vg::core, PropertyFlags, u64,
        ...
        Debug               = 0x0000000000000080,   // A misc debug property that is hidden by default (use 'DebugXXX' categories instead whenever possible)
        ...
        DebugRuntime        = 0x0000000000100000,   // Property is a debug runtime flag that is usually hidden   
        DebugUID            = 0x0000000000200000,   // Property is an debug UID that is usually hidden 
        DebugMatrix         = 0x0000000000400000    // Property is a debug matrix that is usually hidden 
    );
...
//--------------------------------------------------------------------------------------
#define setPropertyFlag(className, propertyName, flags, value)												{ if (auto * prop = _desc.GetLastPropertyByName(#propertyName)) { prop->SetFlags(value ? flags : (vg::core::PropertyFlags)0, value ? (vg::core::PropertyFlags)0 : flags); } else { VG_WARNING("[Factory] Could not set \"Flags\" for property \"%s\" in class \"%s\"", #propertyName, #className); } }
#define setPropertyRange(className, propertyName, range)												    { if (auto * prop = _desc.GetLastPropertyByName(#propertyName)) { prop->SetRange(range);                                                                                  } else { VG_WARNING("[Factory] Could not set \"Range\" for property \"%s\" in class \"%s\"", #propertyName, #className); } }
